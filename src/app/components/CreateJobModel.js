...
const jobSchema = z.object({
  ...
  experience: z.string().nonempty('Experience is required'), // Added experience
...
// The component receives props to control its state and handle events
export default function CreateJobModal({ opened, onClose, onJobCreated }) {
  ...
  // This function will handle form submission later
  const onSubmit = (data) => {
  ...
  const experienceLevels = ['0-1', '1-3', '3-5', '5-8']; // Added experience options
    ...
    <Modal opened={opened} onClose={onClose} title="Create Job Opening" centered size="lg" padding="xl">
      <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <Controller
            ...
            render={({ field }) => (
              <NumberInput
                {...field}
                label=" "
                placeholder="eg., 12" // Changed placeholder
        ...
        <Group justify="space-between" mt="xl">
          ...
          >
            Save Draft <IconChevronDown size={14} style={{ marginLeft: '5px' }} />
          </Button>
          <Button
            type="submit"
            radius="md"
            ...
            style={{ minWidth: '120px' }}
          >
            Publish <IconChevronDown size={14} style={{ marginLeft: '5px' }} />
          </Button>
        </Group>
      </form>
    </Modal>
